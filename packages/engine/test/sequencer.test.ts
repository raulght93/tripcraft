...
const forkById = (id: string) => AFRICA_TRIP.forks.find((f) => f.id === id) as never;

test("ruta por defecto = ACTIVE_PHASES con todos los defaults, sin extensiones", () => {
  assert.deepEqual(buildActivePhaseIds(T), [
    "watamu", "safari-ke", "mafia", "safari-tz-norte", "costa-pemba",
    "mozambique", "victoria", "botswana", "namibia",
  ]);
});
...
test("GOTCHA: skip_uganda/skip_westafrica se OMITEN; skip_f1/skip_f2 PERMANECEN", () => {
  ...
  assert.deepEqual(route, [
    "watamu", "safari-ke", "mafia", "uganda", "safari-tz-norte", "costa-pemba",
    "skip_f1", "victoria", "botswana", "skip_f2", "egypt", "jordan",
  ]);
});
...
test("elegir una opción de fork cambia la fase activa", () => {
  assert.deepEqual(
    ...
    ["lamu", "safari-ke", "mafia", "safari-tz-sur", "costa-pemba", "mozambique", "victoria", "botswana", "namibia"],
  );
});

test("extensiones respetan el orden de members", () => {
  const route = buildActivePhaseIds(T, { extensions: { jordan: true, ethiopia: true, egypt: true } });
...
test("validateReferences detecta valor de recs que no es opción", () => {
  const broken = {
    ...AFRICA_TRIP,
    forks: [{ id: "f", label: "F", options: ["watamu", "lamu"], default: "watamu", recs: { mid: { dive: "diani" } } }],
  };
  const errors = validateReferences(broken as never);
  assert.ok(errors.some((e) => /recs\[mid\]\[dive\]/.test(e)), errors.join("; "));
});

test("validateReferences detecta omitWhenSelected fuera de options", () => {
  const broken = {
    ...AFRICA_TRIP,
    forks: [{ id: "f", label: "F", options: ["watamu"], default: "watamu", omitWhenSelected: ["ghost"] }],
  };
  assert.ok(validateReferences(broken as never).some((e) => /omitWhenSelected/.test(e)));
});

test("el engine es agnóstico: viaje sin IDs de Africa", () => {
  const DEMO = {
